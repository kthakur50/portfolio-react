// ─── Hero Section ──────────────────────────────────────────────────────────
// Landing section: name/role intro on the left, an interactive 3-D skill
// cube on the right.
//
// hero-info-item icons use inline SVGs to avoid an extra icon-library
// dependency. The role text cycles via #slideWords, animated by
// initHeroAnimations() in portfolio.js. #exp-timer is a live IST clock
// updated by initClock() in portfolio.js.
// The 3-D cube (#win3dScene) is built and animated by initWin3DCube() in
// portfolio.js, which exposes a single toggle function on `window` for the
// Rotate button below.
    ...
    <section id="home">
      <div className="wrap">
        <div className="hero-inner">
          ...
          {/* ── Left column: name, role, contact info, action buttons ── */}
          <div className="hero-left sr on">
            ...
            {/* Animated role text — words cycle via initHeroAnimations() */}
            ...
            {/* Quick-glance info: role, location, email, live clock, GitHub */}
            <div className="hero-info sr" style={{ transitionDelay: '.08s' }}>
              <div className="hero-info-grid">
                <div className="hero-info-col">
                  <div className="hero-info-item">
                    {/* Briefcase/arrows icon */}
                ...
                <div className="hero-info-col">
                  <div className="hero-info-item">
                    {/* Clock icon — paired with the live IST timer */}
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--mu)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      <circle cx="12" cy="12" r="10" />
                      <polyline points="12 6 12 12 16 14" />
                    </svg>
                    {/* Filled in every second by initClock() in portfolio.js */}
                  ...
                  <div className="hero-info-item">
                    ...
                    <a href="https://github.com/kaushalt18" target="_blank" rel="noopener">
                      github.com/kaushalt18
                    </a>
                  </div>
                </div>
              </div>
            ...
            <div className="hero-btns">
              <a
                ...
                rel="noopener"
                className="btn-o"
                style={{ padding: '9px 18px', fontSize: '.8rem', borderRadius: '8px' }}
              >
          ...
          {/* ── Right column: interactive 3-D skill cube ──────────────── */}
          <div className="hero-right">
            <div className="win3d-scene" id="win3dScene">
              <div className="win3d-perspective">
                {/* Cube faces are populated dynamically by buildFaces() in
                    initWin3DCube() (portfolio.js) — left empty here on purpose */}
              ...
              <div className="win3d-controls">
                {/* BUGFIX: was calling the bare identifier `wToggleSpin()`, which
                    only worked because portfolio.js happens to attach it as
                    `window.wToggleSpin`. Calling it via `window.` explicitly
                    (with a guard) avoids relying on that implicit global and
                    won't throw if portfolio.js hasn't finished initialising yet. */}
                <button
                  className="win3d-btn"
                  id="wBtnSpin"
                  onClick={() => window.wToggleSpin?.()}
                >
                  Rotate
                </button>
              </div>
